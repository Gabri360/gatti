...
class SearchBox:
    ...
    def draw(self, screen):
        ...
        width, height = self.font.size(text)[0], self.font.get_height()
        while self.font.size(text)[0] > self.size.x * 0.9:
            text = text[1:]
        screen.blit(self.font.render(text, antialias=True, color=self.color_text), (self.pos.x - width / 2, self.pos.y - height / 2))
        alpha = 255 / 2
        for i, p in enumerate(n for n in self.candidates if self.partial in n):
            ...
            srf.set_alpha(alpha)
            screen.blit(srf, (self.pos.x - width / 2, self.pos.y - height / 2 + self.font.get_height() * (i+1)))
            alpha /= 2
